...
import { courseSchema, paymentMethodOptions, paymentMethodLabelsEs } from '@/lib/validation/post';
...
export default function CrearCursoPage() {
  ...
  async function onSubmit(values: CourseFormValues) {
    setSubmitting(true);
    try {
      const payload = { ...values, category: 'cursos' as const };
  ...
  return (
    <div className='mx-auto max-w-3xl px-4 py-8'>
      <div className='mb-6'>
        <h1 className='text-2xl font-semibold'>Publicar curso</h1>
        <p className='text-muted-foreground'>Completa los datos del curso</p>
      </div>
        ...
        <form className='space-y-6' onSubmit={form.handleSubmit(onSubmit)}>
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input {...field} value={field.value ?? ''} onBlur={() => { field.onBlur(); form.trigger('title'); }} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input {...field} value={field.value ?? ''} placeholder='https://...' onBlur={() => { field.onBlur(); form.trigger('image'); }} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input {...field} value={field.value ?? ''} placeholder='https://...' onBlur={() => { field.onBlur(); form.trigger('authorAvatar'); }} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Textarea rows={5} {...field} value={field.value ?? ''} onBlur={() => { field.onBlur(); form.trigger('description'); }} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input {...field} value={field.value ?? ''} onBlur={() => { field.onBlur(); form.trigger('author'); }} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input {...field} value={field.value ?? ''} onBlur={() => { field.onBlur(); form.trigger('location'); }} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
            ...
            {socialsFieldArray.fields.map((f, idx) => (
              <div key={f.id} className='grid gap-3 sm:grid-cols-[1fr_2fr_auto] items-end'>
                ...
                <div className='flex justify-end'>
                  <Button type='button' variant='ghost' disabled={socialsFieldArray.fields.length <= 1} onClick={() => socialsFieldArray.remove(idx)}>
                    Quitar
                  </Button>
                </div>
              </div>
              ...
              <FormItem>
                ...
                <FormControl>
                  <Input
                    ...
                    onBlur={() => { field.onBlur(); form.trigger('price'); }}
                  />
                </FormControl>
                <FormMessage />
              </FormItem>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input {...field} value={field.value ?? ''} onBlur={() => { field.onBlur(); form.trigger('duration'); }} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
                  ...
                  <FormControl>
                    <Input
                      ...
                      onBlur={() => {
                        const v = (field.value ?? '').toString().trim();
                        if (v === '') {
                          form.setValue('schedule', undefined, { shouldValidate: true });
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Select onValueChange={field.onChange} value={(field.value ?? undefined) as any}>
                    <SelectTrigger className='w-full'>
                      <SelectValue placeholder='Seleccionar' />
                    </SelectTrigger>
                    <SelectContent>
          ...
          <div className='flex justify-center'>
            <Button type='submit' disabled={submitting}>
              {submitting ? 'Enviando...' : 'Publicar curso'}
            </Button>
          </div>
        </form>
      </Form>
