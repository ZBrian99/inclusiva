...
import { usedSchema, paymentMethodOptions, paymentMethodLabelsEs } from '@/lib/validation/post';
...
type UsedFormValues = z.infer<typeof usedSchema>;

export default function CrearUsadoPage() {
  const router = useRouter();
  const [submitting, setSubmitting] = useState(false);

  const form = useForm<UsedFormValues>({
  ...
  async function onSubmit(values: UsedFormValues) {
    setSubmitting(true);
    try {
      const payload = { ...values, category: 'usados' as const };
  ...
  return (
		<div className='mx-auto max-w-3xl px-4 py-8'>
			<div className='mb-6'>
				<h1 className='text-2xl font-semibold'>Publicar usado</h1>
				<p className='text-muted-foreground'>Completa los datos del artículo usado</p>
			</div>
				...
				<form className='space-y-6' onSubmit={form.handleSubmit(onSubmit)}>
								...
								<FormControl>
									<Input
										...
										onBlur={() => {
											const v = (field.value ?? '').toString().trim();
											if (v === '') {
												form.setValue('usageTime', undefined, { shouldValidate: true });
												form.clearErrors('usageTime');
											} else {
												field.onBlur();
												form.trigger('usageTime');
											}
										}}
									/>
								</FormControl>
					...
					<div className='flex justify-center'>
						<Button type='submit' disabled={submitting}>
							{submitting ? 'Enviando...' : 'Publicar usado'}
						</Button>
					</div>
				</form>
			</Form>
